...
Page({
  ...
  onLoad: function (options) {
   ...
   console.log("这是我从所有地址列表页面获取过来的receiver"+options.receiver);
   console.log("这是我从所有地址页面获取过来的phone"+options.phone);
   console.log("这是我从所有地址页面获取过来的city"+options.city);
   console.log("这是我从所有地址页面获取过来的province"+options.province);
   console.log("这是我从所有地址页面获取过来的area"+options.area);
   console.log("这是我从所有地址页面获取过来的isdefault"+options.isdefault);

  
    
    if(options.theOrderId)
    {
      that.setData({
    ...
    if(options.receiver)
    {
        ...
        // that.addressinfo.realname=JSON.stringify(options.receiver);
        // that.addressinfo.phone=JSON.stringify(options.phone);
        // that.addressinfo.address=jSON.stringify(options.address);
    }


      //  //组装成一个完整的地址对象 
      //   that.myaddressinfo.realname="张三";
      //   that.myaddressinfo.phone="199999999999999999999";
      //   that.myaddressinfo.address="湖北省武汉市江夏区经济技术开发区";
       // console.log("这是组装后的addressinfo"+that.myaddressinfo.address);

    // that.data.myaddressinfo.prototype.realname="张三";
    // that.data.myaddressinfo.prototype.phone="18888888888";
    // that.data.myaddressinfo.prototype.address="湖北武汉市";
    console.log("----------------------------------66666666666666666666666666666");
    // console.log("组装后的myaddressinfo是"+that.data.myaddressinfo);
  ...
  getPermission: function (obj) {
    ...
    wx.chooseLocation({
      success: function (res) {
        ...
        console.log("获取地址成功!!!" + res.address);
...
// ------------------小程序默认的收货地址-----------------------------------------------------------
        ...
        //   wx.chooseAddress({
        //     success: function (res) {
        //       console.log("获取收货地址--" + JSON.stringify(res));
...
// 小程序默认的收货地址---------------------------------------------------------------------------
  ...
  goMyAllAddress: function (e) {


      this.setData({
        myreceiverid:e.currentTarget.dataset.myreceiverid
      })

    console.log("提交到所有地址列表页面之前的myreceiverid是" + e.currentTarget.dataset.myreceiverid);

    console.log("提交到所有地址列表页面之前的receiver是" + e.currentTarget.dataset.receiver);

    wx.redirectTo({
      url: '../myAllAddress/myAllAddress?myreceiver=' + this.data.myreceiver + '&myreceiverid=' + e.currentTarget.dataset.myreceiverid
    });
  },
  ...
  getAllPayProductList: function () {
    var that = this;
    var myownway = "";
    console.log("你好,这是我从门店自提页面获取过来的订单编号" + that.data.theOrderId);
    wx.request({
      url: theUrl.data.baseUrl + '/mini/order/showpay', // 
      data: {
        storeid: wx.getStorageSync("storeid"),
  ...
  gotopayTheMoney: function () { 
    ...
    console.log("当前的receiver是" + this.data.myreceiver);
    console.log("付款之前的地址中的address是" + this.data.myaddress);
    console.log("付款之前的地址中的province是" + this.data.myprovince);
    console.log("付款之前的地址中的city是" + this.data.mycity);
    console.log("付款之前的地址中的area是" + this.data.myarea);
    console.log("付款之前的地址中的phone是" + this.data.myphone);

    let getMyReceiptinfo = {"realname":this.data.myreceiver, "phone": this.data.myphone, "address":this.data.myaddress};
    this.setData({
      getMyReceiptinfo: getMyReceiptinfo
    });
